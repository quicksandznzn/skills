...
def load_sources() -> list[dict[str, str]]:
    ...
    for source in sources:
        ...
        if not source["purpose"].strip():
            fail(f"Missing purpose for {name}")
        if source["updated_at"] and not DATE_RE.fullmatch(source["updated_at"]):
            fail(f"Invalid updated_at for {name}: {source['updated_at']!r}")
        if bool(source["commit"]) != bool(source["updated_at"]):
            fail(f"commit and updated_at must both be set for {name}")
...
def prepare(source: dict[str, str], target: str, temporary: Path) -> tuple[Path, str, str]:
    ...
    updated_at = run("git", "show", "-s", "--format=%cI", "HEAD", cwd=checkout_dir)[:10]
    return expected, commit, updated_at
...
def sync_docs(sources: list[dict[str, str]], check: bool) -> None:
    rows = []
    for source in sources:
        repository = source["repository"].removesuffix(".git")
        skill_url = f'{repository}/tree/{source["commit"]}/{source["source"]}'
        purpose = source["purpose"].replace("|", "\\|")
        rows.append(
            f'| [{source["name"]}]({skill_url}) | {purpose} | '
            f'{source["updated_at"]} | [GitHub]({repository}) |'
        )
    content = "\n".join(
        [
            "# Upstream skills",
    ...
    current = DOCS.read_text(encoding="utf-8") if DOCS.exists() else ""
    if check and current != content:
        fail(f"{DOCS.relative_to(ROOT)} is out of date; run {Path(__file__).relative_to(ROOT)}")
    if not check and current != content:
...
def main() -> None:
    ...
    for source in sources:
        ...
        with tempfile.TemporaryDirectory(prefix="skills-sync-") as directory:
            expected, commit, updated_at = prepare(source, target, Path(directory))
    ...
    validate_inventory()
    sync_docs(all_sources, args.check)
